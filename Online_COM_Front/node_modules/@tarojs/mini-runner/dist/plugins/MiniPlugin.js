...
class TaroMiniPlugin {
    ...
    generateDarkModeFile(compilation) {
        ...
        if (helper_1.fs.existsSync(themeLocationPath)) {
            const themeLocationStat = helper_1.fs.statSync(themeLocationPath);
    ...
    generateTabBarFiles(compilation) {
        this.tabBarIcons.forEach(icon => {
            ...
            if (helper_1.fs.existsSync(iconPath)) {
                const iconStat = helper_1.fs.statSync(iconPath);
    ...
    injectCommonStyles({ assets }) {
        ...
        const commons = new webpack_sources_1.ConcatSource('');
        ...
        if (commons.size() > 0) {
            ...
            const originSource = assets[appStyle] || new webpack_sources_1.RawSource('');
            assets[APP_STYLE_NAME] = new webpack_sources_1.ConcatSource(originSource);
