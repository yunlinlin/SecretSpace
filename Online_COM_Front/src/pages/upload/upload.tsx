    ...
    if(this.state.selectorChecked === -1){
      return(
        <View className='container'>
          <Picker mode='selector' range={selector} onChange={(e) => this.uploadClass(e)}>
            <AtList>
              <AtListItem
                title='请选择需要上传的类型'
                extraText={selector[this.state.selectorChecked]}
              />
            </AtList>
          </Picker>
        </View>
    ...
    }else if(this.$instance.router?.params.sort === 'activity')
    {
      return (
        <View className='container'>
          ...
          <View className='date-select'>
            <Picker mode='date' value='' onChange={(e) => this.handOnDate(e)} >
              <AtList>
                <AtListItem title='请选择日期' extraText={this.state.dateSelect} />
              </AtList>
            </Picker>
          </View>
          <View className='title'>
      ...
      return(
        <View className='container'>
          ...
          <AtImagePicker 
            files={this.state.files}
            onChange={(files) => this.handOnImage(files)}
          />
          <View className='submit' onClick={() => this.uploadDetail()}>
            <Text>提交</Text>
          </View>
